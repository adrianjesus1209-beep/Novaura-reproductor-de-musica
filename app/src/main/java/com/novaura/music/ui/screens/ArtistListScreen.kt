...
fun ArtistListScreen(
    ...
    val artistGroups = remember(uiState.songs, searchQuery) {
        uiState.songs
            .groupBy { it.artist }
            .map { (artist, songs) -> ArtistGroup(artist, songs) }
            .filter { group ->
                searchQuery.isBlank() || group.name.contains(searchQuery, ignoreCase = true) ||
                        group.songs.any { it.title.contains(searchQuery, ignoreCase = true) }
            }
    ...
    Scaffold(
        ...
        Column(
            ...
            if (uiState.songs.isNotEmpty()) {
                ...
                Text(
                    text = stringResource(R.string.artists_count, artistGroups.size),
                            ...
                            Column(modifier = Modifier.fillMaxWidth()) {
                                Row(
                                    modifier = Modifier
                                        .fillMaxWidth()
                                        .clickable {
                                            expandedArtists[group.name] = !isExpanded
                                        }
                                        .padding(16.dp),
